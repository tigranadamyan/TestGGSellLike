...
{
    /**
     * The 50-key pool supplied in the assignment appendix. A key can never go to
     * two orders, so the pool is dealt out across the key-bearing SKUs rather
     * than duplicated per product.
     */
    private const POOL = [
    ...
    {
        $products = Product::whereIn('type', ['key', 'giftcard', 'subscription'])
            ->orderBy('id')
            ->get();
        ...
        foreach (self::POOL as $i => $code) {
            $product = $products[$i % $products->count()];

            $product->keys()->firstOrCreate(
                ['code' => $code],
                ['status' => ProductKeyStatus::Available]
            );
        }

        // Keep the denormalised storefront counter in step with the pool.
