...
Проще всего — открыть **`/stand`**: там те же сценарии выполняются кнопками, с живым
состоянием склада и разбором того, что должно произойти. Терминал не нужен.
Маршруты `/stand/*` меняют данные каталога, поэтому отключены в production.
...
GET    /stand                        — Стенд для проверки: сценарии кнопками, без терминала
